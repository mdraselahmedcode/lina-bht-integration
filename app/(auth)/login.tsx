...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Detect "not verified" error and redirect to verify screen
      const isNotVerified =
      ...
      if (isNotVerified) {
        ...
        // Resend OTP automatically so the code is fresh
        try {
          await resendOtp({ email: formData.email, purpose: 'verify_email' }).unwrap();
        } catch {
          // Silently ignore resend failure — user can resend manually on next screen
        }
        router.push({
  ...
  const handleSocialLogin = (provider: string) => showInfo(`${provider} login coming soon!`);
