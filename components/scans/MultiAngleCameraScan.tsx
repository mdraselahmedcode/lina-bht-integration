...
// Destructure with a fallback to avoid "not found" crashes
...
// File system helpers - using the correct Expo FileSystem API
// Ensure documentDirectory is not null (it can be null on web)
const BASE_DIR = documentDir ?? '';
const CAPTURES_DIR = `${BASE_DIR}${BASE_DIR.endsWith('/') ? '' : '/'}face_captures/`;
...
const saveImageToFileSystem = async (uri: string, angle: string): Promise<string> => {
  await ensureCapturesDir();

  const timestamp = Date.now();
  const filename = `${angle}_${timestamp}.jpg`;
  const newPath = CAPTURES_DIR + filename;
...
export const MultiAngleCameraScan: React.FC<MultiAngleCameraScanProps> = ({
  ...
  useEffect(() => {
    if (isComplete && capturedAngles.length > 0 && !hasNavigated) {
      ...
      const capturesToStore = capturedAngles
        .filter((capture) => capture.localPath)
        .map((capture) => ({
          angle: capture.angle,
  ...
  const takePicture = React.useCallback(async () => {
    ...
    try {
      ...
      const localPath = await saveImageToFileSystem(photo.uri, currentAngle);
